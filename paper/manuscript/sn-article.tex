...
\begin{table}[]
\centering
\caption{Configuration of U-Net-like Upsampling Path, where ops represent a transpose convolution path in Fig \ref{fig1-modelA}, and $n$ is the number of transpose convolution filters}
\label{tab-unetpath}
...
\end{tabular}%
\footnotetext{Note: All 2D transpose convolution are done with kernel's size $2\times2$ and stride 2 which causes an enlarging factor of 2.}
\end{minipage}
\end{table}

\subsubsection{Pixel Classification Branch}
Three pixel prediction blocks are proposed, named ``Pxl Header1", ``Pxl Header2", and ``Pxl Header3" in Fig \ref{fig1-modelA}. Table \ref{tab-pxlheader} outlines the function layers on each block, where all block has the same compoments. Tensors with size $224 \times 224 \times 64$, which come from the last layer of the upsampling network, are simultaneously fed into all pixel prediction block.
