...
Three pixel prediction blocks are proposed, named ``Pxl Header1", ``Pxl Header2", and ``Pxl Header3" in Fig \ref{fig1-modelA}. Table \ref{tab-pxlheader} outlines the function layers on each block, where all block has the same compoments. A tensor which size $224 \times 224 \times 64$, that comes from the last layer of the upsampling network, is simultaneously fed into every pixel prediction blocks. 
...
For inferencing a tapping line, the results from every pixel prediction blocks are passd to the function $\mathrm{g}(x)$. The function $\mathrm{g}(x)$ composed of three component. Firstly, it finds the pixel location with the highest probability on each column (columnwise-argmax) of each input, then checks for the majorities (2 of 3), and binarizes the values on every pixels, then finally passes them to the pixel output. 

In the network traning phase, network losses are computed on each outputs of pixel prediction blocks, which means function $\mathrm{g}(x)$ and beyonds are not contribute in loss calculation. ``Pxl Header" combines low-level and high-level features via any mapping function, which can be arbitary specified in network traning phase. This will be explain in detail and discussed in later sections.
