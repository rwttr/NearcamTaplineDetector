...
The downsampling backbone network borrows residual block arrangement of Darknet-53. The network is trimmed out the numbers of filters and shrank the overall depth, as shown in table \ref{tab-darknetlight}, while preserving filter's sizes and LeakyReLU activation function. 

\begin{table}[]
\centering
\caption{tablebackbone}
...
Box detection ends comply the YOLO's method. Two YOLO detection branches connect to the downsampling network at the output of block ``down4" and ``down5", referred to table \ref{tab-darknetlight}. Each branch has private $3\times3$ and $1\times1$ (pointwise convolution) filters that remaps tensor's depth to $5\times$ no.of anchors, where each anchor requires four for coordinates and one for the objectness scores, detailed in table \ref{}.
