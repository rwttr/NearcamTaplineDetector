...
To archieve learning regularization or to prevent the model from overfitting, a couple of tools are presented. Traditional L2-regularization is used on the weights of the downsampling network. Random rotations within $\pm20^\circ$ and the magnifications within $\pm20\%$ apply on the input images for the augmentation purpose. Another tool is checking the prediction on YOLO outputs. If the box predictions reach 0.8 IoU with the groundtruth, then the box's loss calculation in that grid cell of YOLO feature maps will be ignored.

\subsection{Evaluation Method}
Annotating a tapping line with a bounding box and a polyline inside enables the detection in many aspects and levels.  The bounding box can justify the existence of a tapping line which usualy indicates by using box overlapping ratio, IoU. 
