...
\subsection{Pixel Overlapping and Distances}
What does the tapping line detection that result in a high Hausdorff distance with also high F1-scores imply? An actual example is in Fig \ref{}, the results come from model B and model C that trained with Focal pixel clasification loss, perform on the same test image. 

\subsection{Multiple Pixel Prediction Heads with non-linear function $\mathrm{g}(x)$}
\section{Conclusion}
