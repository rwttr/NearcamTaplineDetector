...
iou_penalty_threshold = 0.75     # prediction iou to ignore loss calculate (yolo branch)
...
ps = params(model)
# param by block
ps_downsampling = Flux.params(model.basenet_block)
ps_upsampling = Flux.params(model.upsample_block)
ps_yolohead = Flux.params(model.yolohead_block)
ps_pxlhead = Flux.params(model.pxpredhead_block)
...
@epochs no_epoch begin
    ...
    while current_data_iter <= max_epoch_iter
        ...
        g = Flux.gradient(ps) do
            ...
            lossPxl_3 = pxlLoss_focal(response_pxl_3, target_pxl)
                    
            reg_all = λ_downsample* L2reg(ps_downsampling) +
               λ_upsample * L2reg(ps_upsampling) +
               λ_yolohead * L2reg(ps_yolohead) +
               λ_pxlhead * L2reg(ps_pxlhead)
            ;

            loss = lossBox_1 + lossObj_1 + lossBox_2 + lossObj_2 + 
                lossPxl_1 + lossPxl_2 + lossPxl_3 + reg_all
            ;
